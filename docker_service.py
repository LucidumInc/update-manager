...
def _pull_docker_image_from_ecr(repository: str, tag: str):
...
def _pull_docker_image_from_docker_hub(repository: str, tag: str):
    docker_client.images.pull(repository, tag=tag)


def pull_docker_image(repository: str, tag: str) -> Image:
    ...
    return docker_client.images.get(f"{repository}:{tag}")

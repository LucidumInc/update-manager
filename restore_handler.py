...
class MongoRestoreRunner(BaseRestoreRunner):
    ...
    def __call__(self):
        ...
        restore_cmd = f"mongorestore -v --username={{mongo_user}} --password={{mongo_pwd}} --authenticationDatabase=test_database --host={{mongo_host}} --port={{mongo_port}} --archive={self.container_dest_dir}/{os.path.basename(local_filepath)} --gzip --db={{mongo_db}} --numInsertionWorksPerCollection=4 --drop"
        try:
